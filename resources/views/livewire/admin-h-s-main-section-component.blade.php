            ...
            <div class="h-full px-3 py-4 overflow-y-auto bg-blue-50 dark:bg-gray-800">
                ...
                <ul class="space-y-2 font-medium">
                    @foreach($items['classes'] as $keyClass => $item)                
                        <li>
                        <button wire:click="activeClass('{{ $keyClass}}')" class="flex items-center p-2 text-gray-900 cursor-pointer rounded-lg dark:text-white hover:bg-blue-100 dark:hover:bg-gray-700 group {{ $items['classes'][$keyClass]['is_active'] == true ? 'bg-blue-400 dark:bg-blue-700' : '' }}">
                        ...
                        @if($items['classes'][$keyClass]['is_active'] == true)
                            ...
                            @foreach($item['semesters'] as $keySemester => $semester)   
                                <li>
                                    <button 
                                        wire:click="activeClassSemester('{{$keyClass}}', '{{$keySemester}}')"
        ...
        <main class="w-full mx-auto px-0 py-0 overflow-y-auto rounded bg-gray-200 dark:bg-gray-800 min-h-screen">            
            ...
            <div class="flex flex-row items-start gap-2 my-4 mx-2">
                @foreach($statusButtons as $key =>$button)
                    <button
                        wire:click="changeButtonStatus('{{$key}}')" 
            ...
            @else
                ...
                @elseif($statusButtons['admission'] == true)
                    @livewire('admin-hs-cls-sec-wise-admitted-students',['hsClassId' => 2, 'hsSectionId' => 1, 'hsSemesterId' => 1]);
                @elseif($statusButtons['promotion'] == true)
                    @livewire('admin-hs-cls-sec-wise-promoted-students',['hsClassId' => 2, 'hsSectionId' => 1, 'hsSemesterId' => 1]);
                @endif
